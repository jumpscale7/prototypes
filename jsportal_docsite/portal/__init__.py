...
class Page:
    ...
    def load(self):
        hrdfile=self.path.replace(".md",".hrd")
        if j.system.fs.exists(hrdfile):
            hrd=j.core.hrd.get(hrdfile)
            self.args=hrd.getHRDAsDict()

        content = render_template_string(self.raw_content, page=self,**self.args) #this is to render the markdown
...
def render_page(path):
    ...
    #this does further template processing
    return render_template(page.meta['template'], content=page.html_content, page=page,**page.args)
